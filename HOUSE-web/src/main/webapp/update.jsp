...
<script type="text/javascript">
	...
	function isImage() {
		$.post("HouseAction_isImage.action", function(data) {
			$("#tdd22").html(data);
		});
	}
</script>
		...
		<form action="HouseAction_updateHouse.action" method="post"
			...
			<table>
				...
				<tr height="40px">
					...
					<td><input type="file" id="upload" name="upload" value="${houses.image}" /></td>
				</tr>
				<tr height="40px">
					<td align="right">图 片 标 题：</td>

					<c:forEach items="${pictures}" var="housep">
						<td><input type="text" id="photoTitle" name="picture.title"
							size="45px" value="${housep.title}" /></td>
					</c:forEach>
				</tr>
				<tr>
					<td align="right">已上传图片：</td>
					<td id="tdd22"><img id="img" src="image/${houses.image}" width="171px"
						height="151px"></td>
				</tr>
				<tr height="40px">
					<td align="right">联 系 方 式：</td>
					<td><input type="text" id="telephone" name="house.contact"
						size="45px" value="${houses.contact}" /></td>
				</tr>
				<tr height="40px">
					<td align="right">详 细 信 息：</td>
					<td><textarea rows="6" cols="39" name="house.description"
							value="${houses.description}"></textarea></td>
				</tr>
				<tr height="80px">
					<td colspan="2" align="center"><input type="button"
						class="submit" value="立即修改" onclick="subaddHouse()" /></td>
				</tr>
			</table>
		</form>
	</div>
